...
def solution():
    ...
    for x in range(0, 400):
        ...
        # vertical
        if (x + 60) < len(array): 
            product = int(array[x]) * int(array[x + 20]) * int(array[x + 40]) * int(array[x + 60])
            if product > solution:
                solution = product
        
        # horizontal
        if int(x/20) == int((x + 3)/20): 
            product = int(array[x]) * int(array[x + 1]) * int(array[x + 2]) * int(array[x + 3])
            if product > solution:
                solution = product

        # diagonal
        if (x + 60 + 3) < len(array) and int((x + 60 + 3)/20) == int((x + 60)/20): 
            product = int(array[x]) * int(array[x + 21]) * int(array[x + 42]) * int(array[x + 63])
            if product > solution:
                solution = product 

        # diagonal reverse
        if (x + 60) < len(array) and int((x + 60 - 3)/20) == int((x + 60)/20): 
            product = int(array[x]) * int(array[x + 19]) * int(array[x + 38]) * int(array[x + 57])
            if product > solution:
                solution = product 
